...
def validate_move(move, matrix):
    if 0 <= move[0] < len(matrix) and 0 <= move[1] < len(matrix[0]) and matrix[move[1]][move[0]].check_availability(): # Podmienka na validovanie kroku, overuje,
...
def start_with_move_and_printing_and_stepper():
    ...
    result = move_timer_stepper_move_limit(board, field, calculate_possible_moves(board, field), order_counter, moves, #Funkcia pomocných módov programu
                                           start_time)
...
def start_task_f():
    ...
    for position in positions:
        ...
        result = move_simple(board_5x5, current_field, calculate_possible_moves(board_5x5, current_field), # rekurzívna funkcia (DFS)
                             order_counter,
                             moves,
                             start_time)
        if result is not False:
            print("Riešenie existuje")
            dummy_matrix = [[board_5x5[j][i].order_number for j in range(len(board_5x5[0]))] for i in #Šachovnica s poradovými číslami
...
def move_timer_stepper_move_limit(board, current_field, move_vectors, order_counter, moves, start_time): #Funkcia pre pomocné módy programu
...
def move_simple(board, current_field, move_vectors, order_counter, moves, start_time):
    ...
    if moves <= MAX_MOVES: #Overenie pre limit krokov
        if order_counter == len(board) * len(board[0]): #Overenie, či program neskončil vyhľadávanie a nemá už celé riešenie
            return moves
...
def move_just_get_result_single_problem(board, current_field, move_vectors, order_counter, moves, start_time): #Funkcia pre pomocné módy
    time_elapsed = time.time() - start_time
    moves += 1
    if moves <= MAX_MOVES:
        if order_counter == len(board) * len(board[0]):
...
if __name__ == "__main__":
    ...
    move_vectors = [(1, 2), (2, 1), (2, -1), (1, -2), (-1, -2), (-2, -1), (-2, 1), (-1, 2)] #Reprezentácia pohybov vektormi
